...
export function MainTabs({params}: MainTabsProps) {
  ...
  const {data: userTasks, isLoading: tasksLoading} = useSWR<TaskWithProject[]>(
  ...
  const getTasksByStatus = (tasks: TaskWithProject[] | undefined, status: string) => {
    if (!tasks) return [];
  ...
  const getUpcomingTasks = (tasks: TaskWithProject[] | undefined) => {
    ...
    return tasks
      .filter(task => {
        ...
        return dueDate >= now && dueDate <= oneWeekFromNow && task.status !== 'DONE';
      })
      .sort((a, b) => {
  ...
  const handleProjectTaskClick = useCallback((taskId: string) => {
    if (!projectId) return;
    ...
    if (projectId && project) {
      ...
      const ganttTasks = project.tasks
        ...
        .map(task => ({
          ...
          endDate: task.dueDate ? new Date(task.dueDate) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
      ...
      return (
        <div className='space-y-6 p-6'>
          <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center gap-2'>
              <Badge className={`${project.status === 'IN_PROGRESS' ? 'bg-blue-500' :
          ...
          <Card className='p-6'>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <p>{project.startDate ? new Date(project.startDate).toLocaleDateString('ja-JP') : '未設定'}</p>
              </div>
              <div>
                <p className='text-sm text-muted-foreground'>終了日</p>
                <p>{project.endDate ? new Date(project.endDate).toLocaleDateString('ja-JP') : '未設定'}</p>
              </div>
              <div>
